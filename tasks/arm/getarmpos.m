function [ x ] = getarmpos(angles,link_lengths,ticks,plot_me)
...
x_links = zeros(n_dofs+1,n_tt,2);

for tt=compute_ticks'
  sum_angles = 0;
  for dof=1:n_dofs
    sum_angles = sum_angles + angles(dof,tt);
    x_links(dof+1,tt,1) = x_links(dof,tt,1) + cos(sum_angles)*link_lengths(dof);
    x_links(dof+1,tt,2) = x_links(dof,tt,2) + sin(sum_angles)*link_lengths(dof);
  end
end

x = squeeze(x_links(end,ticks,:))';

% Plotting
if (plot_me)
  % End-effector path
  plot(squeeze(x_links(end,ticks,1)),squeeze(x_links(end,ticks,2)),'ko-','LineWidth',1)
  hold on
